...
function OpenWindowError() {
    ...
    function WindowErrorSound() { const w = ["https://vanhcutii.store/window-error.mp3",];let i=Math.floor(Math.random()*w.length);const a=new Audio(w[i]);function p(){a.play()}document.addEventListener("click",p()),a.addEventListener("ended",function(){i=(i+1)%w.length,a.src=w[i],a.play()});}
        // Count Love Days
